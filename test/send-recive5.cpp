...
void thread_fun(int thread_id) {
    ...
    infinity::memory::Buffer *sendBuffer = new infinity::memory::Buffer(context, BUFFER_SIZE);
    infinity::memory::Buffer *reciveBuffer = new infinity::memory::Buffer(context, BUFFER_SIZE);
    ...
    for (uint64_t i = 0; i < ECHO_COUNT; i++) {
        memset(sendBuffer->getData(), 0, BUFFER_SIZE);
...
void server_thread(std::unique_ptr<infinity::core::Context> context, std::unique_ptr<infinity::queues::QueuePair> qp){
    ...
    infinity::memory::Buffer *sendBuffer = new infinity::memory::Buffer(context.get(), BUFFER_SIZE);
    infinity::memory::Buffer *receiveBuffer = new infinity::memory::Buffer(context.get(), BUFFER_SIZE);
...
int main(int argc, char **argv) {
    ...
    if(isServer) {
        ...
        for(auto i = 0; i < thread_count; i++) {
            vector_[i] = std::thread(thread_fun, i);
        }
